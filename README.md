...
agent_stonks/
  ...
  apple_trader.py — Apple Trader: rule-based (no LLM) loop on the day-range forecast. It rests
                  a buy and a sell at fixed distances below the forecast high (per-instrument
                  defaults) and rebuilds both whenever a session that trades outside the
                  forecast moves it
