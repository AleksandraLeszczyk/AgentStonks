...
- **🍎 Apple Trader (rule-based, no LLM)** — the one personality with no model reasoning in the loop: once a minute it reads the minute bar that just closed and applies fixed rules built on a saved model, so the same tape always produces the same trades. The model is the **Day-range forecast (TimeToChange3)**: at **9:35** it forecasts where the *whole session's* high and low will land, from a year of daily bars plus the first five 1-minute bars, and then asks the model nothing else all day. Three predictors blend equally (LightGBM + N-BEATS + N-HiTS over a 32-day lookback of eight per-day channels), a heavily-regularised ridge learns what the opening five minutes add on top of the daily prediction, and the result is clipped to contain the range that has already printed. **Fitted per ticker**: `Models/timetochange3_dayrange_<TICKER>.joblib` (AAPL, GOOGL and INTC) plus the two `.pt` checkpoints and the metadata JSON beside each, every symbol with its own daily models, its own opening ridge and its own held-out error ($2.12, $2.75 and $1.72 respectively). Needs `torch`, `lightgbm`, `scikit-learn` and `joblib`; locations override with `APPLE_DAYRANGE_MODEL_<TICKER>`. Not selectable by the Automatic orchestrator, which drives LLM prompts
  ...
  **Which instrument** is picked in the same panel, and the model constrains it: every rule here is a saved model's output, so the picker offers only the symbols the model was fitted on. A run trades that one symbol, which must be streamed. A missing model or dependency is reported instead of traded around.
  ...
  The rules are TimeToChange3 notebook 05's, with both levels adjustable. With **H** the predicted high and **A** the trailing 14-day average daily range in dollars, it **buys** when a bar's low reaches `H − buy × A` and **sells** when a bar's high reaches `H − sell × A`, re-arming as often as the day allows, with anything still open flattened before the close. The levels default per instrument — AAPL 0.40/0.25, GOOGL 0.65/0.05, INTC 0.50/0.05 — from re-running the notebook's own grid over every session with a forecast (`FinNotebooks/TimeToChange3/scripts/sweep_levels.py`); the notebook itself specified 0.75/0.10, which any other symbol falls back to. On top of that sits a **managed exit**, all of it measured from the fill and each part switchable off with 0: a **stop** that sells when a bar's low reaches `fill − stop × (buy − sell) × A` and ends new entries for the session — written as a share of the **predicted gain**, the gap between the two levels and the most a target exit can pay, so the default **0.5** risks $0.50 for every $1.00 the trade is playing for on every instrument. (In ADRs the same number would not travel: 0.20 ADR was a third of AAPL's 0.15-ADR target and a third of GOOGL's 0.60-ADR one at once. On the shipped pairs 0.5 of the gain comes to 0.075 ADR on AAPL, 0.30 on GOOGL and 0.225 on INTC — so it is a much tighter stop on AAPL than the 0.20 ADR it replaces, and a wider one on GOOGL. The form says what the fraction comes to for the pair currently set.) The two levels are both `reference − k × A` off the same number, so the gap between them never moves however the reference does — which is what lets a share of it be a fixed price once there is a fill. SimLab records written before this replay under the ADR distance they were run at and sign as `stop=E-0.2A` rather than being re-read into the new unit, which would be a different stop filed beside the original; a **momentum take** that, once the tape's momentum score has fallen `drop` σ (1.0) from its best since the entry while the position is in profit, sells 70% and keeps the rest as a **runner** — but only when the sell level is still at least `hold × A` (0.30) above the fill, otherwise it sells everything; and a **breakeven** on that runner, sold if the price comes back to the fill. On top of that sits a **session circuit breaker**: once a trade has closed for no more than `min win × A` **per share** (0.20), the agent buys nothing else that day — a round trip that barely paid is evidence the setup was not there this session. It is judged over the whole position, so a momentum take and the runner it left count as one trade, and it catches what the stop does not: the trades that were not losses but were not worth the risk either, a runner sold back at the fill most of all. It has to be read against `buy − sell`, the most a target exit can net (0.15 on AAPL, 0.60 on GOOGL, 0.45 on INTC): at or above that, *every* completed trade stands the session down, which is a one-trade-a-day rule rather than a circuit breaker. So the threshold is **per instrument** like the levels are — **AAPL 0.10**, everything else 0.20 — which leaves a target exit alive on all three, and the form warns if the two are ever set to disagree. None of the exit or the breaker is in the notebook's numbers, and SimLab records from before them replay with them off. It is a mean-reversion bet by construction: what the model forecasts well is the **width** of the day, not its direction, so the rule buys well below where the day is expected to top out and sells just under it, and on a day that never dips that far it does nothing.
  ...
  **H is not quite fixed for the day either.** The model runs once — its opening features are a fixed five-minute window, so it cannot be re-run — but a session that trades *through* the predicted high has already falsified it, and both levels hang off that number. What happens then is a setting with three choices. **Hold the 9:35 forecast** is the notebook's rule and what every stored record replays as. **Move to the extreme so far** replaces the breached side with the session's own high (or low) — the same claim `apply_open_constraint` already makes against the opening five minutes, carried through the day. **Brownian extension** goes past the extreme by `A × √(session left) ÷ 2`, the one-sided excursion a driftless walk whose volatility is implied by the ADR would still be expected to make (`E[range] = 2 × E[max]` over a session is what makes the constant a half): half an ADR with the whole day ahead, nothing at the bell. Only the breached side moves, only ever outward, and both levels are rebuilt from the new high the moment it moves — including for a position already open, whose target moves away from it. The choice is part of the Results signature unless it is off, so the three queue as three configurations; neither updating policy has been swept, and the buy/sell distances above were picked with the forecast held fixed all day. Note that the two settings are much less than additive: under the intraday reference a dollar added to the predicted high moves the levels by the shape at that minute, about a fifth of a dollar in the afternoon.
  ...
  What the numbers are. On a 129-session test window the blend's mean absolute error on the two targets is **0.0077 in log units ($2.12)**, against 0.0110 for a 14-day rolling baseline and 0.0135 for persistence — 30% of the baseline's error removed, and the ordering holds across all four walk-forward refits. Divided by a normal day's range that is **42% of one ADR**, which is how the ML Models tab leads with it and the unit the levels below are already written in; SimLab's Drift tab takes the same ratio session by session, against each session's own ADR. That is a statement about the forecast, not about the trading rule: the default levels were picked on the same 31–36 sessions they are scored on, with limit fills and no costs, and how far each deserves trust differs by ticker (INTC's pick flips sign between the two halves of its sample) — `config.APPLE_TRADER_DAYRANGE_LEVELS` carries the table.

  Two things the live path does differently from the notebook, both worth knowing before comparing results. The **fill**: the notebook rests limit orders and fills a touch *at* the level, while this ledger is market-order only and buys near the close of the bar that touched it, so a bar that dipped and recovered fills worse here — a real cost, and it runs one way. And the **minute tape's volume scale**: the opening ridge's `or_volume_share` feature was fitted on consolidated volume, and Alpaca's IEX feed carries about 4% of it, which puts that one feature outside anything it saw in training. Live, the forecast no longer takes what it is given — the opening five minutes are sourced the way the bar history is, consolidated tape first (SIP, delayed SIP, yfinance) and **IEX only when none of them will serve the 09:30 window**, which in practice means only before ~09:50 on a free key. A consolidated tape is preferred even when the buffer already holds the right five minutes on the IEX scale. The caveat, when it appears, names the tape the forecast was actually built on rather than whichever Alpaca feed the sidebar is holding — so a Finnhub run (the default, consolidated) no longer carries an IEX warning it never earned. SimLab has no such fallback: a replay reads the tape its dataset was downloaded on, so download one on `yfinance` or `sip`, and the caveat now reports that feed instead of the live default.
...
- **Tuning tab** — a parameter grid for Apple Trader, after TimeToChange3 notebook 05's profit heatmap. Pick **one or two parameters** to sweep (the day-range buy/sell distances, the stop, the momentum fade and take, the runner threshold, the session circuit breaker, the size or the flatten window) with a from/to/step each; everything else comes from a base configuration edited with the same form as Simulate. Pick a **tuning dataset** and a **test dataset**: the grid is swept on the first, one cell is chosen there — the highest, or the middle of the best 3×3 plateau as `sweep_levels.py` chose the shipped levels, among cells that traded on enough of the days — and that pick is replayed on the second next to the untuned base configuration, with a verdict on whether its profit held up. Optionally the whole grid is swept on the test dataset too, which shows whether the profitable region itself moved. Every cell is a real replay through the same engine and trader as Simulate — market fills, managed exit, flatten — so a cell's number is what that configuration would show in a Simulate run (lower than the notebook's limit-fill heatmap). Combinations the configuration refuses (a sell distance at or above the buy distance) are left as holes, datasets sharing sessions are flagged as not out of sample, and sessions the model could not forecast are named. A grid is rarely a blank sheet, so before anything is queued the form fills in every cell a **stored simulation run already answers** and leaves the rest transparent — an empty square means "not tried", never "tried and flat". A cell *is* a Simulate run of that configuration, so a stored run stands in for one when it matches the base in every setting the grid does not sweep, and shares the sessions, the tape and the starting cash; those cells are seeded into the job and the worker only replays the holes, with each cell's origin shown in the combinations table. A run is only reused while it is still *current*: modification times say whether anything it read — the dataset's bars and news, the symbol's daily history, the market indicators, the model bundle — has changed since it was saved, because the daily history is a rolling store shared by every dataset and refreshing it moves the day-range forecast, and so both levels and every fill. It cannot see a change in code, so reworking the trader's rules wants the affected runs deleted or **Reuse stored runs** switched off. A job runs in a detached worker (`python -m simlab.tuning <id>`) that fans the remaining cells out over a process pool; the tab shows its progress, can stop it (keeping the cells already swept), and stores results under `data/simlab/tuning/`. The rule replay was made several times faster to afford this — the market's daily-bar and day-volume reads bisect a per-day index instead of rescanning the tape every minute, and the rule agents' minute frame is sliced from one frame per replayed day — each pinned against the code it replaced

- **Summary tab** — the whole run store at a glance: three cards for the best (or, with the same control flipped, the **worst**) single runs under the filters, ranked on return or on profit efficiency with the other always shown beside it, over a breakdown table and bar charts by agent, ML model, instrument or dataset. The worst end is worth the same glance as the best — a configuration that loses reliably is as much a finding as one that wins, and it is the end nobody scrolls a table to find. Each card **opens its run in the Results tab**, clearing the filters there on the way so the run is actually reachable: Results keeps its own filter set, and a run Summary can see may be one Results has filtered out, whose picker would then quietly open a different run.

  Two consequences of making that possible, both worth knowing. The tab bar is a **stateful widget** now, which is what lets a card switch tabs at all — a plain `st.tabs` accepts the session-state write and the frontend ignores it. That in turn makes the tabs **lazy**: only the open tab's body runs, so opening SimLab no longer scores every drift model and parses every stored run before drawing the first tab. And because a scheduler that only ticks while one tab is open is a queue that stalls the moment you go and read a result, the **experiment pipeline moved above the tab bar** — it is the app's queue, not the Simulate tab's, and experiments run whatever is on screen.

- **Apple Trader in SimLab** — the rule-based agent replays on the same engine, the same tape and the same ledger, but on its own day loop: it reads *every* closed bar of the session instead of sleeping on armed conditions, so there is nothing to fast-forward past. It needs no LLM, so it is queued once per dataset rather than once per LLM model, and its **rule set stands in for a model name** — moving either day-range level, what they are measured below, the intraday update, the circuit breaker, the size or the instrument is a new configuration to test, not a repeat of one already run. **It is never scored by the LLM judge**: the judge grades an agent's stated reasoning against the tape it cited, and this one states none of its own, so profit, profit efficiency and the oracle ceiling are its whole scorecard. It requires the symbol its configuration names in the dataset — checked before anything is queued, and part of the signature, so the same levels over AAPL and over GOOGL are two configurations to compare rather than one averaged row. It reads the dataset's stored **daily** history too (a 252-day window, well inside the 420 days a dataset stores), plus the simulated day's official opening print — the one field of that day's stored daily bar exposed during the run, since the 9:30 auction is fixed before the forecast is made. Stored runs on the removed momentum models still appear in Results, but a replay of one is refused
...
- **Sweeping settings inside one Apple Trader setup** — a setup is one configuration until you tell it to vary something. **Vary these settings** takes any of the ten that matter — the two levels, what they hang off, the breach policy, the stop, the momentum take and its runner, the session circuit breaker, the size — and multiplies the setup out into one experiment per combination, each queued against every selected dataset and each its own row in Results. A **number** gets the same from/to/step the Tuning tab uses, so a range means the same thing in both places; a **rule** gets a multiselect of its options, since "hold the forecast, move to the extreme, or extend it" is a set and not an interval. Each axis says how many values it came to and what the base setup holds, which is the one thing worth knowing while building a grid.

  Three things a grid does that a single setup cannot, all reported rather than absorbed. A cell can be **refused** — a buy distance under the sell distance is not a strategy, and the reason is printed rather than the row quietly vanishing. Cells can **collapse**: vary the take fraction with the momentum take switched off and the signature cannot tell them apart, so they are queued once and the page says how many merged. And a grid can be far bigger than anyone meant, so a setup is capped at **64 configurations** — over that it is queued as its base configuration alone until the grid is narrowed, because each one is a worker process, a run record and a Results row, not a cell in a heatmap. The count next to the Run button counts configurations, so the multiplication on screen is the number of experiments you are about to queue.

  This is the Tuning tab's grid without its pick: there the point is a heatmap over two numeric axes and one cell chosen on a tuning dataset and re-tested on another, here it is a handful of named configurations replayed side by side and compared in Results. Both describe a grid the same way and share one table of what may be varied, so a range that is legal in one is legal in the other. `flatten_before_close_min` is tunable and deliberately **not** sweepable here — Results identifies a configuration by its signature and the flatten is not in it, so varying it would queue several runs that show up as one row.
...
agent_stonks/
  ...
  model_overlays.py — what the saved models predict, as drawing instructions: one catalogue of
                  ...
                  × day range, plus Apple Trader's own buy/sell levels -- the one that draws an
                  agent rather than a model), each computed from bars the caller supplies, so the live chart
  ...
  apple_trader.py — Apple Trader: rule-based (no LLM) loop on the day-range forecast. It rests
                  ...
                  minute -- rebuilds both whenever either the reference or the clock moves,
                  and stands the session down after a stop or a trade that barely paid
...
simlab/
  ...
  tuning.py     — Apple Trader parameter tuning: a one- or two-parameter grid swept on a tuning
                  ...
                  beside the base configuration; jobs run detached over a process pool, and
                  cells a stored simulation run already answers are reused rather than replayed
